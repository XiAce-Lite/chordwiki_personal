...
> 日常的な画面操作や機能の使い方は `USER_GUIDE.md` を参照してください。  
> ただし、**セットアップ手順の正本はこの `README.md`** です。AI セットアップエージェントは、引き続き本 README の記載順を優先してください。
...
曲データに加え、**セットリスト**（`/setlists.html`、専用コンテナ `setlists`）を Cosmos DB に保存します。画面操作は `USER_GUIDE.md` も参照してください。
...
```json
{
  ...
  "Values": {
    ...
    "COSMOS_DB_CONTAINER_SONGS": "Songs",
    "COSMOS_DB_CONTAINER_SETLISTS": "setlists"
  }
}
```
...
- **曲用コンテナ** 名: `Songs`、Partition Key: `/artist`
- **セットリスト用コンテナ** 名: `setlists`、Partition Key: `/userId`  
  （API の既定値。環境変数 `COSMOS_DB_CONTAINER_SETLISTS` で変更可）
...
| `COSMOS_DB_CONTAINER_SONGS` | 曲コンテナ名（`Songs`） |
| `COSMOS_DB_CONTAINER_SETLISTS` | セットリストコンテナ名（`setlists`） |
...
- `COSMOS_ENDPOINT` / `COSMOS_KEY` / `COSMOS_DB_NAME` / `COSMOS_DB_CONTAINER_SONGS` / `COSMOS_DB_CONTAINER_SETLISTS` を設定した
