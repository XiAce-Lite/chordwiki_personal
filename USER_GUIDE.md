...
- **可変スクロール切替**：`可変スクロール` の ON/OFF（OFF 時は等速モード）
- **表示カスタマイズ**：コードサイズ、記号縦位置、コード行縦位置、行間などの見た目調整
- 各ブロックは **▼ / ▶** で縦に折りたたみ可能
...
2. 再生したい時間を設定する
3. `可変スクロール` を必要に応じて切り替える（OFF で等速モード）
4. `Start` を押して再生する
5. 必要なら再生中に速度を微調整する（最大 `3.00x`）
6. `End` マーカーの上端がビューポート下端から 100px 内側に入った時点で自動停止する

既定の再生時間（約 4 分前後）のまま曲を開いた場合、**曲長の自動推定**（iTunes / MusicBrainz / YouTube のいずれかを参照）が 1 回だけ走り、推定に成功すると分・秒入力とステータス表示が更新されます。手動で時間を変えたあとは自動では上書きしません。
...
- トップページの **「セットリスト」** から `/setlists.html` を開きます
- 複数のセットリストを作成し、曲をドラッグで並べ替えたり、リスト名を保存できます
- **共有**にチェックを入れたリストは、他のログインユーザーにも一覧に表示されます（閲覧のみの相手には編集ボタンは出ません）
- セットリストの作成・名前保存・削除・共有切替は **`editor` ロール**が必要です
